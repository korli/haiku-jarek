...
#if defined(_KERNEL_MODE)
static spinlock guard_slock = B_SPINLOCK_INITIALIZER;
#else
...
_LIBCXXABI_FUNC_VIS int __cxa_guard_acquire(guard_type *guard_object) {
    char* initialized = (char*)guard_object;
#if defined(_KERNEL_MODE)
    int cpu_flags = arch_int_disable_interrupts();
    acquire_spinlock(&guard_slock);
#else
...
#if defined(_KERNEL_MODE)
    release_spinlock(&guard_slock);
    arch_int_restore_interrupts(cpu_flags);
#else
