...
#if defined(_KERNEL_MODE)
                    for(;;) panic("terminating with %s exception of type %s: %s",
                                  cause, name, e->what());
#else
...
#if defined(_KERNEL_MODE)
                    for(;;) panic("terminating with %s exception of type %s",
                                   cause, name);
#else
...
#if defined(_KERNEL_MODE)
                for(;;) panic("terminating with %s foreign exception", cause);
#else
